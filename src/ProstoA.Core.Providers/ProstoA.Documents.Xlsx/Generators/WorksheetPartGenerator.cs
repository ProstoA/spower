...
namespace ProstoA.Documents.Xlsx.Generators {
    internal sealed class WorksheetPartGenerator {
        public WorksheetPart[] Do(WorkbookPart workbookPart, params XlsxWorksheet[] documentWorksheets) {
            return documentWorksheets.Select((x, index) => Do(workbookPart, x, $"rId{index}")).ToArray();
        }
        ...
        public WorksheetPart Do(WorkbookPart workbookPart, XlsxWorksheet documentWorksheet, string id) {
            ...
            worksheet.Append(
                ...
                new PageSetup { PaperSize = 9U, FitToHeight = 0U, Orientation = documentWorksheet.Orientation },
